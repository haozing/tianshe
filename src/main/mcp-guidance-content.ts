  ...
  switch (guideName) {
    case 'login_pages':
      return buildGuideFrame('Airpa Login Pages', [
        ...
        '',
        'Recommended flow:',
        '1. `profile_ensure_logged_in` first when a reusable profile should carry the login state or the page asks for authentication.',
        '2. If the result needs manual login, use the prepared visible session for human handoff; do not ask the model to type passwords, one-time codes, CAPTCHA, recovery codes, or token values.',
        '3. After the human completes the login, use `browser_observe` or `browser_snapshot` to verify the success route or success text.',
        '4. Use `browser_act` only for non-secret consent or submit steps that remain after the human-controlled login flow.',
        '',
        'Heuristics:',
        '- Prefer reusing an existing logged-in profile before any new login handoff.',
    ...
    default:
      return buildGuideFrame('Airpa MCP Getting Started', [
        ...
        '',
        'Key defaults:',
        '- Prefer this `airpa-browser-http` MCP surface when the task must reuse an Airpa-managed logged-in profile, desktop session, or sticky browser state.',
        ...
        '- For site-specific extraction or repeatable site automation, call `site_capability_list` before generic browser tools and prefer the matching `<site>.<action>` capability when available.',
        ...
        '- For authentication, re-authentication, CAPTCHA, or 2FA states, call `profile_ensure_logged_in` and use visible human handoff before browser automation continues.',
        ...
        '- For repeatable read-only extraction, prefer a Site Adapter fixture run when one exists; inspect `site_adapter_result`, `site_adapter_failure`, and `site_adapter_repair_evidence` artifacts instead of improvising page code.',
        '- Treat Site Adapter repair as scoped to adapter extractors, verifiers, fixtures, and expected outputs; do not modify framework core as part of a repair.',
...
export const buildInitializeInstructions = (
  ...
  return [
    ...
    'Recommended flow:',
    ...flow,
    'Prefer this Airpa HTTP MCP surface when you need a real Airpa profile, logged-in state, or visible desktop browser state.',
    ...
    'Use `site_capability_list` before generic browser tools for site-specific extraction; prefer a matching `<site>.<action>` capability when one exists.',
    ...
    'Use `profile_ensure_logged_in` for authentication, re-authentication, CAPTCHA, or 2FA states; use visible human handoff instead of model-entered secrets.',
    ...
    'For repeatable read-only extraction, prefer Site Adapter fixture evidence and inspect `site_adapter_result`, `site_adapter_failure`, and `site_adapter_repair_evidence` artifacts before proposing repairs.',
    'Keep Site Adapter repairs scoped to adapter extractors, verifiers, fixtures, and expected outputs; do not repair framework core in this path.',
...
export const buildPromptMessageText = (
  ...
  return [
    ...
    'For site-specific extraction, call site_capability_list first and prefer a matching <site>.<action> capability over generic browser scripting.',
