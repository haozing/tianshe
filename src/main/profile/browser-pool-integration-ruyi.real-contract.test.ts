...
const shouldRunRealContract =
  process.env.AIRPA_RUN_RUYI_REAL_CONTRACT === '1' ||
  process.env.AIRPA_RUN_RUYI_REAL_CONTRACT === 'true';
...
function logStep(step: string): void {
  if (debugRealContract) {
    console.log(`[ruyi-real-contract] ${step}`);
  }
}

describe('createRuyiBrowserFactory shared real contract', () => {
  runRealContract(
    ...
    async () => {
      ...
      const tempRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'airpa-ruyi-real-contract-'));
      ...
      try {
        ...
        }, 5_000, 'ruyi emulated user agent');
        ...
        }, 5_000, 'ruyi emulation clear user agent');
        {
        ...
        report.emulationClearedUserAgent =
          (await created.browser.evaluate<string>('navigator.userAgent')) === baselineUserAgent;
        logStep('emulation contract verified for ruyi runtime');
