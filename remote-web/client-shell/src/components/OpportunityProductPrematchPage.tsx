...
function Metric({ label, value, detail, tone = "default" }: { label: string; value: string; detail: string; tone?: "default" | "blue" | "green" | "warn" }) {
  ...
  return (
    <div className="min-w-0 rounded-md border border-[#edf1f6] bg-[#f4f6fa] px-3 py-1.5">
      <span className="block truncate text-[12px] font-semibold text-[#475467]">{label}</span>
      <strong className={cn("block truncate text-[20px] font-bold leading-6 tracking-[0]", toneClass)}>{value}</strong>
      <span className="block truncate text-[11px] text-[#667085]">{detail}</span>
    </div>
  );
}
...
function SelectField<T extends string | number>({ label, value, options, onChange }: {
  ...
  return (
    <label className="grid min-w-[132px] gap-1.5">
      ...
      <select
        className="h-9 rounded-md border border-[#d7dee9] bg-white px-2.5 text-[13px] font-medium text-[#1d2939] outline-none transition-colors focus:border-[#2f6df6]"
        value={String(value)}
        onChange={(event) => {
  ...
  return (
    <div className="grid gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          className={cn("h-8 rounded-md border px-2.5 text-[12px] font-semibold", values.length === 0 ? "border-[#2f6df6] bg-[#eef4ff] text-[#175cd3]" : "border-[#d7dee9] bg-white text-[#526a91] hover:border-[#2f6df6]")}
          type="button"
          onClick={onClear}
        >
          不限
        ...
        {options.map((item) => {
          ...
          return (
            <button
              className={cn("h-8 rounded-md border px-2.5 text-[12px] font-semibold", active ? "border-[#2f6df6] bg-[#eef4ff] text-[#175cd3]" : "border-[#d7dee9] bg-white text-[#526a91] hover:border-[#2f6df6]")}
...
function CheckOption({ checked, label, onChange }: { checked: boolean; label: string; onChange: (checked: boolean) => void }) {
  return (
    <label className="inline-flex h-8 cursor-pointer items-center gap-2 rounded-md border border-[#d7dee9] bg-white px-2.5 text-[12px] font-semibold text-[#344054] hover:border-[#2f6df6]">
      <input className="size-4 accent-[#2f6df6]" checked={checked} type="checkbox" onChange={(event) => onChange(event.target.checked)} />
      {label}
    </label>
  );
}
    ...
    <table className="min-w-[1180px] w-full border-separate border-spacing-0 text-left">
      <thead className="sticky top-0 z-10 bg-[#fbfcff] text-[12px] font-semibold text-[#667085]">
        <tr>
          <th className="w-10 border-b border-[#edf1f6] px-3 py-2.5">
            <input aria-label="选择全部候选" checked={allChecked} className="size-4 accent-[#2f6df6]" type="checkbox" onChange={onToggleAll} />
          </th>
      ...
      <tbody className="text-[13px]">
        {items.map((item) => {
          ...
          return (
            <tr className="group hover:bg-[#f8fbff]" key={item.id}>
              <td className="border-b border-[#edf1f6] px-3 py-3 align-top">
                <input aria-label={`选择${item.title}`} checked={selectedIds.has(item.id)} className="size-4 accent-[#2f6df6] disabled:opacity-40" disabled={!selectable} type="checkbox" onChange={() => onToggle(item.id)} />
              </td>
              <td className="border-b border-[#edf1f6] px-3 py-3 align-top">
              ...
              <td className="border-b border-[#edf1f6] px-3 py-3 align-top">
                <span className={cn("inline-flex rounded-md px-2 py-1 text-[12px] font-semibold", item.eligible && item.status === "ready" ? "bg-[#eafaf0] text-[#087443]" : item.status === "submitted" ? "bg-[#eef6ff] text-[#175cd3]" : "bg-[#fff7e8] text-[#b54708]")}>
                  {statusText(item)}
                </span>
...
export function OpportunityProductPrematchPage() {
  ...
  return (
    <section className="h-full min-h-0 overflow-auto bg-[#f4f7fb] px-6 py-5 text-[#101828]" data-business-slot="ready">
      <div className="mx-auto flex w-full max-w-[1420px] flex-col gap-4">
        <section className="rounded-lg border border-[#dbe3ee] bg-white p-2.5 shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
          <div className="grid grid-cols-6 gap-2 max-[1280px]:grid-cols-3 max-[720px]:grid-cols-1">
        ...
        <section className="grid gap-4 rounded-lg border border-[#dbe3ee] bg-white p-4 shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
          <div className="flex items-center justify-between gap-3 border-b border-[#edf1f6] pb-3 max-[900px]:items-start max-[900px]:flex-col">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="m-0 text-[16px] font-bold tracking-[0] text-[#101828]">提报工作台</h2>
                <button className="inline-flex h-8 items-center gap-1.5 rounded-md border border-[#d7dee9] bg-white px-2.5 text-[12px] font-semibold text-[#344054] hover:border-[#2f6df6] hover:text-[#175cd3] disabled:opacity-50" type="button" onClick={restoreLatest} disabled={busy}>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span className={cn("rounded-md border px-2.5 py-1 text-[12px] font-semibold", item.active ? "border-[#b8cdfd] bg-[#eef4ff] text-[#175cd3]" : "border-[#e4e7ec] bg-[#f9fafb] text-[#667085]")} key={item.label}>{item.label}</span>
              ))}
            </div>
          </div>

          <div className="grid grid-cols-[360px_minmax(0,1fr)_360px] gap-4 max-[1260px]:grid-cols-[330px_minmax(0,1fr)] max-[940px]:flex max-[940px]:flex-col">
            <section className="flex h-[430px] min-h-0 flex-col overflow-hidden rounded-md border border-[#dfe6ef] bg-white">
              <div className="flex min-h-[44px] items-center justify-between border-b border-[#dfe6ef] bg-[#fbfcff] px-3">
                <strong className="text-[14px] text-[#101828]">店铺范围</strong>
                <div className="flex items-center gap-2">
                  <button className="inline-flex h-7 items-center gap-1 rounded-md border border-[#d7dee9] bg-white px-2 text-[12px] font-semibold text-[#344054] hover:border-[#2f6df6] hover:text-[#175cd3] disabled:opacity-50" type="button" onClick={refreshStores} disabled={busy}>
                  ...
                  <label className="flex items-center gap-2 text-[12px] font-semibold text-[#475467]">
                    <input aria-label="全选店铺" checked={stores.length > 0 && selectedShopIds.size === stores.length} className="size-4 accent-[#2f6df6]" type="checkbox" onChange={selectAllStores} />
                    全选
                  </label>
                </div>
              </div>
              ...
              <div className="min-h-0 flex-1 overflow-auto">
                {stores.map((store, index) => {
                  ...
                  return (
                    <label className="grid min-h-[42px] cursor-pointer grid-cols-[44px_minmax(0,1fr)_96px] items-center border-b border-[#edf1f6] text-[13px] text-[#344054] hover:bg-[#f8fbff]" key={store.shopId}>
                      <span className="flex items-center justify-center gap-2">
                        <input checked={selectedShopIds.has(store.shopId)} className="size-4 accent-[#2f6df6]" type="checkbox" onChange={() => toggleStore(store.shopId)} />
                      </span>
                      <span className="min-w-0 truncate px-3 font-semibold" title={store.shopName}>{index + 1}. {store.shopName}</span>
                      <span className="px-2 text-center">
                        <span className={cn("inline-flex rounded-md px-2 py-0.5 text-[12px] font-semibold", candidateCount ? "bg-[#eef4ff] text-[#175cd3]" : productCount ? "bg-[#eafaf0] text-[#087443]" : "bg-[#f2f4f7] text-[#667085]")}>{status}</span>
                      </span>
                    </label>
                  );
                })}
            ...
            <section className="grid gap-3 rounded-md border border-[#dfe6ef] bg-[#fbfcff] p-4">
              <div className="grid gap-3 rounded-md border border-[#edf1f6] bg-white p-3">
                <div className="flex items-center justify-between gap-3">
                  <strong className="text-[14px] text-[#101828]">1. 数据准备</strong>
                  <span className="rounded-md bg-[#f2f4f7] px-2 py-1 text-[12px] font-semibold text-[#667085]">官方接口</span>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <button className="inline-flex h-9 items-center gap-1.5 rounded-md bg-[#2f6df6] px-3 text-[13px] font-semibold text-white hover:bg-[#255fd9] disabled:opacity-50" type="button" onClick={syncProducts} disabled={busy || !selectedShopIds.size}>
                    {loading === "products" ? <Loader2 className="size-[15px] animate-spin" strokeWidth={2} /> : <PackageSearch className="size-[15px]" strokeWidth={2} />}
                    同步商品
                  </button>
                  <button className="inline-flex h-9 items-center gap-1.5 rounded-md border border-[#d7dee9] bg-white px-3 text-[13px] font-semibold text-[#344054] hover:border-[#2f6df6] hover:text-[#175cd3] disabled:opacity-50" type="button" onClick={scanClues} disabled={busy || !selectedShopIds.size}>
              ...
              <div className="grid gap-3 rounded-md border border-[#edf1f6] bg-white p-3">
                <strong className="text-[14px] text-[#101828]">2. 筛选规则</strong>
                <div className="grid grid-cols-[150px_150px_minmax(0,1fr)] gap-3 max-[1180px]:grid-cols-2 max-[760px]:grid-cols-1">
                  ...
                  <label className="grid gap-1.5">
                    ...
                    <input className="h-9 rounded-md border border-[#d7dee9] px-2.5 text-[13px] font-semibold text-[#1d2939] outline-none focus:border-[#2f6df6]" min={1} max={10000} type="number" value={dailyAttemptLimit} onChange={(event) => setDailyAttemptLimit(Math.max(1, Number(event.target.value || 1)))} />
                  </label>
                </div>
                <div className="grid gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <button className={cn("h-8 rounded-md border px-3 text-[13px] font-semibold", matchMode === item.key ? "border-[#2f6df6] bg-[#eef4ff] text-[#175cd3]" : "border-[#d7dee9] bg-white text-[#344054] hover:border-[#2f6df6]")} key={item.key} type="button" onClick={() => setMatchMode(item.key)}>
                        {item.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {rankTabs.map((item) => (
                      <button className={cn("h-8 rounded-md border px-2.5 text-[12px] font-semibold transition-colors", activeRank === item.value ? "border-[#2f6df6] bg-[#eef4ff] text-[#175cd3]" : "border-[#d7dee9] bg-white text-[#526a91] hover:border-[#2f6df6]")} key={item.value} type="button" onClick={() => setActiveRank(item.value)}>
                        {item.label}
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid gap-3 rounded-md border border-[#edf1f6] bg-white p-3">
                <div className="flex items-center justify-between gap-3 max-[760px]:items-start max-[760px]:flex-col">
                  <strong className="text-[14px] text-[#101828]">3. 生成与提报</strong>
                ...
                <div className="flex flex-wrap items-center gap-2 border-t border-[#edf1f6] pt-3">
                  <button className="inline-flex h-10 items-center justify-center gap-1.5 rounded-md border border-[#2f6df6] bg-white px-4 text-[14px] font-semibold text-[#175cd3] hover:bg-[#eef4ff] disabled:opacity-50" type="button" onClick={generatePrematch} disabled={busy || !productRunId || !clueRunId}>
                    {loading === "match" ? <Loader2 className="size-[16px] animate-spin" strokeWidth={2} /> : <Shuffle className="size-[16px]" strokeWidth={2} />}
                    生成预匹配
                  </button>
                  <button className="inline-flex h-10 items-center justify-center gap-1.5 rounded-md bg-[#2f6df6] px-4 text-[14px] font-semibold text-white hover:bg-[#255fd9] disabled:opacity-50" type="button" onClick={submitPrematch} disabled={busy || !selectedCandidates.length}>
            ...
            <section className="flex h-[430px] min-h-0 flex-col overflow-hidden rounded-md border border-[#dfe6ef] bg-white max-[1260px]:col-span-2 max-[940px]:col-span-1">
              <div className="flex min-h-[44px] items-center justify-between border-b border-[#dfe6ef] bg-[#fbfcff] px-3">
                <strong className="text-[14px] text-[#101828]">运行日志</strong>
              ...
              <div className="min-h-0 flex-1 overflow-auto bg-[#fcfdff] p-3 font-mono text-[12px] leading-5 text-[#175cd3]">
                {busy ? (
        ...
        <section className="flex min-h-[360px] flex-col overflow-hidden rounded-lg border border-[#dbe3ee] bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
          <div className="flex min-h-[48px] items-center justify-between gap-3 border-b border-[#edf1f6] px-4">
            <div className="flex min-w-0 items-center gap-2">
              <Play className="size-[16px] shrink-0 text-[#2f6df6]" strokeWidth={2} />
              <strong className="text-[15px] text-[#101828]">预匹配候选明细</strong>
            ...
            <div className="flex h-9 w-[300px] max-w-[40vw] items-center gap-2 rounded-md border border-[#d7dee9] bg-white px-2.5 focus-within:border-[#2f6df6] max-[760px]:hidden">
