...
const requiredWriteActions = Object.fromEntries(features.map((feature) => [feature, forcePhase4 ? writeActionsByFeature[feature] : marketingCapability?.features?.[feature]?.writeActions || []]));
...
const fixtureResults = files.map((filePath) => {
  try {
    ...
    return { file: rel(filePath), feature: "", action: "", phase: "", scenario: "", entityIds: [], ok: false, issues: [error instanceof Error ? error.message : String(error)] };
  }
});
const observedCases = new Set(fixtureResults.filter((item) => item.ok).map((item) => `${item.feature}:${item.action}:${item.phase}:${item.scenario}`));
const requiredCases = marketingRequested ? [
  ...
  ...features.flatMap((feature) => requiredWriteActions[feature].flatMap((action) => ["mutation", "reconcile"].flatMap((phase) => scenarios.map((scenario) => `${feature}:${action}:${phase}:${scenario}`))))
] : [];
const missingCases = requiredCases.filter((key) => !observedCases.has(key));
const duplicateCases = [];
const caseCounts = new Map();
for (const item of fixtureResults) {
  const key = `${item.feature}:${item.action}:${item.phase}:${item.scenario}`;
  caseCounts.set(key, (caseCounts.get(key) || 0) + 1);
}
for (const [key, count] of caseCounts) if (key !== "::" && count > 1) duplicateCases.push(key);
...
const failed = failedFixtures.length > 0 || missingCases.length > 0 || duplicateCases.length > 0 || missingEvidence.length > 0;
const report = {
  schemaVersion: 1,
  checkedAt: new Date().toISOString(),
  status: failed ? "fail" : "ok",
  readiness: marketingRequested ? "required" : "pending",
  fixtureRoot: rel(fixtureRoot),
  fixtureCount: files.length,
  requiredCaseCount: requiredCases.length,
  requiredWriteActions,
  missingCases,
  duplicateCases,
  missingEvidence,
...
console.log(JSON.stringify({ status: report.status, readiness: report.readiness, fixtureCount: report.fixtureCount, missingCaseCount: missingCases.length, missingEvidenceCount: missingEvidence.length, failedFixtureCount: failedFixtures.length, outputPath: rel(outputPath) }, null, 2));
