...
export default class MenuBuilder {
  ...
  buildMenu(): Menu {
    if (isDevelopmentMode() || AIRPA_RUNTIME_CONFIG.app.debugProdMenu) {
  ...
  setupDevelopmentEnvironment(): void {
    this.mainWindow.webContents.on('context-menu', (_, props) => {
      ...
      ]).popup(); // Electron 30+: popup() 会自动使用当前聚焦的窗口
    });
  }
  ...
  buildDarwinTemplate(): MenuItemConstructorOptions[] {
    ...
    const subMenuView =
      isDevelopmentMode() || AIRPA_RUNTIME_CONFIG.app.debugProdMenu
        ? subMenuViewDev
        : subMenuViewProd;
