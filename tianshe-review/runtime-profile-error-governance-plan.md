...
- 截至本批，`src/main/duckdb` 生产代码已基本完成 direct console baseline 清理；`src/main/ipc-handlers` 生产代码已无直接 `console.*`，`SchedulerService`、main service composition、main entry、query template service、duckdb service/utils/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service/export plan builder/dataset folder/type analyzer/dataset import service/query facade/dataset facade/dataset service coordinator/small services/import worker、window manager、小 main/profile/webhook 模块、browser pool runtime、main/preload 散点、core small runtime、FFI service/library、JS plugin small runtime、query builders，以及 webcontentsview controller 组也已清零；后续重点转向 profile/browser-pool canary/shared 页面脚本信号、core query/js-plugin 旧热点和 renderer 少量 direct console。
