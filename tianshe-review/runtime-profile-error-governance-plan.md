...
- 截至本批，`src/main/ipc-handlers` 生产代码已无直接 `console.*`；IPC handler 层的 logger baseline 已清零，后续重点转向 main bootstrap、duckdb dataset/query service、scheduler/webcontentsview/window-manager 等非 IPC 热点。
...
- [ ] 阶段 5：按模块递减 logger baseline。（已完成 account/saved-site namespace、DuckDB account/profile-group/saved-site/tag、IPC wrapper、profile/browser IPC、dataset route、JS plugin route/handler、account IPC、query template IPC、tag/extension packages IPC、dataset folder IPC、registration-only IPC、file IPC、system IPC 小批次）
