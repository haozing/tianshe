...
- 截至本批，`src/main/ipc-handlers` 生产代码已无直接 `console.*`，`SchedulerService`、main service composition、main entry、query template service、duckdb service/utils/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service/export plan builder/dataset folder/type analyzer/dataset import service/query facade、window manager、小 main/profile/webhook 模块，以及 webcontentsview controller 组也已清零；后续重点转向其他 duckdb dataset 子服务/facade/import-worker、profile/browser-pool integration 和 renderer 少量 direct console。
...
- [ ] 阶段 5：按模块递减 logger baseline。（已完成 account/saved-site namespace、DuckDB account/profile-group/saved-site/tag/utils/service/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service/export plan builder/dataset folder/type analyzer/dataset import service/query facade、IPC wrapper、profile/browser IPC、dataset route、JS plugin route/handler、account IPC、query template IPC/service、tag/extension packages IPC、dataset folder IPC、registration-only IPC、file IPC、system IPC、scheduler service、main service composition、main entry、window manager、small main/profile/webhook、webcontentsview controller 小批次）
