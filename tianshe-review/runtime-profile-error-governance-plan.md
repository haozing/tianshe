...
本文初始调研时的生产路径粗略扫描，不含测试和 `__tests__`，共有 178 个文件、1241 处直接 `console.log/warn/error/info/debug` 调用；该表保留为治理前基线，后续完成情况以阶段 5 记录和 `DIRECT_CONSOLE_CALL_BASELINE` 为准。当时前 25 个热点如下：
...
- 已完成 JS plugin handler 小批次：`src/main/ipc-handlers/js-plugin-handler.ts` 的 21 处直接 `console.*` 已迁移到 `createLogger('JSPluginIPCHandler')`，并从 baseline 移除。
- 已完成 account IPC 小批次：`src/main/ipc-handlers/account-ipc-handler.ts` 的 11 处直接 `console.*` 已迁移到 `createLogger('AccountIPCHandler')`，并从 baseline 移除；本批只迁日志，不改变登录/popup 返回语义。
- 已完成 query template IPC 小批次：`src/main/ipc-handlers/query-template-handler.ts` 的 22 处直接 `console.*` 已迁移到 `createLogger('QueryTemplateIPCHandler')`，并从 baseline 移除；本批只迁日志，不改变查询模板 CRUD/query 返回语义。
...
- 已完成 system IPC 小批次：`src/main/ipc-handlers/system-handler.ts` 的 43 处直接 `console.*` 已迁移到 `createLogger('SystemIPCHandler')`，覆盖 app info、device fingerprint、download-image 和 shell 路径日志，并从 baseline 移除。
- 已完成 scheduler service 小批次：`src/main/scheduler/scheduler-service.ts` 的 27 处直接 `console.*` 已迁移到既有 `createLogger('SchedulerService')`，覆盖定时器、错过执行、重试、完成/失败和清理日志，并从 baseline 移除；同时修复一处损坏注释吞掉 interval `nextRunAt` 赋值的问题，资源租约和重试语义不变。
- 已完成 main service composition 小批次：`src/main/bootstrap/main-service-composition.ts` 的 26 处直接 `console.*` 已迁移到 `createLogger('MainServiceComposition')`，覆盖核心服务、窗口服务、IPC 路由、插件运行时、proxy cleanup 和 browser pool 初始化日志，并从 baseline 移除；本批不改变启动顺序和 readiness 标记。
- 已完成 query template service 小批次：`src/main/duckdb/query-template-service.ts` 的 28 处直接 `console.*` 已迁移到 `createLogger('QueryTemplateService')`，覆盖模板 CRUD、快照刷新、默认模板修复和排序日志，并从 baseline 移除；本批不改变 SQL 生成、快照表命名和默认模板修复语义。
- 已完成 duckdb utils 小批次：`src/main/duckdb/utils.ts` 的 25 处生产 direct `console.*` 已迁移到 `createLogger('DuckDBUtils')`，覆盖临时文件清理、DuckDB 值转换、重试、完整性检查/修复和事务 rollback 日志，并从 baseline 移除；JSDoc 示例中的 `console.error` 保留为文档示例，不计入架构护栏。
- 已完成 window manager 小批次：`src/main/window-manager.ts` 的 18 处直接 `console.*` 已迁移到 `createLogger('WindowManager')`，覆盖主窗口、popup、hidden automation host、resize callback 和 cleanup 日志，并从 baseline 移除；本批不改变窗口生命周期和 resize debounce 行为。
- 已完成 small main/profile/webhook 小批次：`src/main/file-storage.ts` 的 9 处、`src/main/webhook/sender.ts` 的 9 处、`src/main/sync/sync-outbox-emitter.ts` 的 2 处、`src/main/profile/browser-launcher.ts` 的 2 处、`src/main/profile/chrome-runtime-shared.ts` 的 1 处直接 `console.*` 已迁移到模块 logger，并从 baseline 移除；profile 页面脚本内用于 canary/探针的 console 输出本批不动。
- 已完成 webcontentsview manager/attachment/viewport 小批次：`src/main/webcontentsview-manager.ts` 的 17 处、`src/main/webcontentsview-attachment-controller.ts` 的 9 处、`src/main/webcontentsview-viewport-debugger.ts` 的 1 处直接 `console.*` 已迁移到模块 logger，并从 baseline 移除；本批不改变 view 注册/激活、attachment/offscreen、viewport 诊断触发语义。
- 已完成 webcontentsview lifecycle 小批次：`src/main/webcontentsview-lifecycle-controller.ts` 的 33 处直接 `console.*` 已迁移到 `createLogger('WebContentsViewLifecycleController')`，覆盖 preload、bounds reapply、background throttling、权限拦截、导航、view cleanup、debugger detach 和 webContents destroy 日志，并从 baseline 移除；本批不改变视图创建/关闭顺序、权限策略和 delayed destroy 行为。
- 已完成 webcontentsview state 小批次：`src/main/webcontentsview-state-controller.ts` 的 27 处直接 `console.*` 已迁移到 `createLogger('WebContentsViewStateController')`，覆盖 batch close、oldest close、plugin view activation、reserve/release、mark busy/idle/error 和 GC 日志，并从 baseline 移除；本批不改变 view state、reservation 和 GC 行为。
- 已完成 webcontentsview layout 小批次：`src/main/webcontentsview-layout-controller.ts` 的 23 处直接 `console.*` 已迁移到 `createLogger('WebContentsViewLayoutController')`，覆盖 plugin dock restore、right dock bounds、window resize、page/temp/pool bounds 更新和 displayMode 日志，并从 baseline 移除；本批不改变 dock/resize/layout 计算行为。
...
- 已完成 duckdb service facade 小批次：`src/main/duckdb/service.ts` 的 23 处直接 `console.*` 已迁移到 `createLogger('DuckDBService')`，覆盖 DuckDB 初始化、WAL replay recovery、系统表初始化、初始化 checkpoint、默认查询模板 ATTACH 和 close/final checkpoint 日志，并从 baseline 移除；本批不改变服务初始化顺序、WAL 备份/恢复、checkpoint 和 close 清理行为。
- 已完成 duckdb dataset metadata 小批次：`src/main/duckdb/dataset-metadata-service.ts` 的 24 处直接 `console.*` 已迁移到 `createLogger('DatasetMetadataService')`，覆盖 plugin metadata checkpoint、getDatasetInfo/schema parse、metadata delete、schema update/reorder 和 type analysis 日志，并从 baseline 移除；本批不改变 metadata SQL、transaction、schema JSON 和 type analyzer 调用行为。
- 已完成 duckdb dataset record mutation 小批次：`src/main/duckdb/dataset-record-mutation-service.ts` 的 10 处直接 `console.*` 已迁移到 `createLogger('DatasetRecordMutationService')`，覆盖 hard delete、row_count increment/decrement、insert/update/batch mutation 日志，并从 baseline 移除；本批不改变事务、row_count 同步和 hookBus 事件行为。
- 已完成 duckdb dataset query 小批次：`src/main/duckdb/dataset-query-service.ts` 的 12 处直接 `console.*` 已迁移到 `createLogger('DatasetQueryService')`，覆盖 queryDataset SQL preview/result、Aho-Corasick attach/load/build/batch/summary 和空词库日志，并从 baseline 移除；本批不改变查询分页、readonly SQL 校验、computed column 包装和 AC 匹配行为。
- 已完成 duckdb dataset schema 小批次：`src/main/duckdb/dataset-schema-service.ts` 的 16 处直接 `console.*` 已迁移到 `createLogger('DatasetSchemaService')`，覆盖计算列依赖、验证规则、复制数据、列 add/update/delete、未知字段类型、特殊字段默认值和默认值填充日志，并从 baseline 移除；本批不改变列名策略、schema 元数据、物理列 ALTER、依赖检查和“失败不中断”的警告语义。
- 已完成 duckdb dataset export writer 小批次：`src/main/duckdb/dataset-export-writer.ts` 的 17 处直接 `console.*` 已迁移到 `createLogger('DatasetExportWriter')`，覆盖 CSV/TXT/Parquet/JSON/Excel 导出开始完成、Excel 拆分、流式写入、单文件写入和写文件失败日志，并从 baseline 移除；本批不改变 COPY SQL、Excel 分片、进度回调、编码重写和错误抛出语义。
- 已完成 duckdb dataset export service 小批次：`src/main/duckdb/dataset-export-service.ts` 的 6 处直接 `console.*` 已迁移到 `createLogger('DatasetExportService')`，覆盖导出开始、SQL preview、完成、失败、post-export action 和永久删除日志，并从 baseline 移除；本批不改变导出计划构建、writer 调用、进度回调、失败返回 envelope 和物理删除语义，完整 SQL 日志收缩为 preview。
- 已完成 duckdb dataset folder 小批次：`src/main/duckdb/dataset-folder-service.ts` 的 13 处直接 `console.*` 已迁移到 `createLogger('DatasetFolderService')`，覆盖 folder create/update/delete、dataset move、排序、插件文件夹 bootstrap、缺失 folder_id 跳过和失败日志，并从 baseline 移除；本批不改变文件夹树构建、事务删除、插件目录保护和 bootstrap “失败不中断”语义，同时清理一个未使用 insert 返回值。
- 已完成 duckdb type analyzer 小批次：`src/main/duckdb/type-analyzer.ts` 的 12 处直接 `console.*` 已迁移到 `createLogger('TypeAnalyzer')`，覆盖采样规模、列统计、single_select、file/link 检测、attachment/hyperlink 推断日志，并从 baseline 移除；本批不改变动态采样、类型推断、URL/文件识别和颜色映射行为，同时不再输出真实样本值/选项值。
- 已完成 duckdb dataset import service 小批次：`src/main/duckdb/dataset-import-service.ts` 的 16 处直接 `console.*` 已迁移到 `createLogger('DatasetImportService')`，覆盖 worker path fallback、文件大小检查、worker 启动/退出/错误、取消终止、artifact/metadata/temp cleanup、row_count increment、cross-database insert 和 temp detach 日志，并从 baseline 移除；本批不改变 worker 生命周期、progress 回调、rejectWithCleanup、跨库插入和清理容错语义。
- 已完成 duckdb dataset export plan builder 小批次：`src/main/duckdb/dataset-export-plan-builder.ts` 的 4 处直接 `console.*` 已迁移到 `createLogger('DatasetExportPlanBuilder')`，覆盖 query-backed export SQL 重建、生成和系统列/ORDER BY 包装降级日志，并从 baseline 移除；本批不改变列投影、隐藏列合并、selectedRowIds 过滤、rowIdSQL 和 ORDER BY hoist 行为。
- 已完成 duckdb query facade 小批次：`src/main/duckdb/duckdb-service-query-facade.ts` 的 5 处直接 `console.*` 已迁移到 `createLogger('DuckDBServiceQueryFacade')`，覆盖 export SQL 去分页构建/完成和 dataset attach 日志，并从 baseline 移除；本批不改变 QueryEngine 调用、依赖 attach、LIMIT/OFFSET 移除和 exportDataset 转发行为。
- 已完成 duckdb dataset facade 小批次：`src/main/duckdb/duckdb-service-dataset-facade.ts` 的 6 处直接 `console.*` 已迁移到 `createLogger('DuckDBServiceDatasetFacade')`，覆盖 import 完成后 tab group/default template 补齐、datasetService 未初始化和 type analysis 委派日志，并从 baseline 移除；本批不改变 observation span、import progress 包装、默认模板失败不中断、query tracing 和 DatasetService 转发行为。
- 已完成 duckdb dataset service coordinator 小批次：`src/main/duckdb/dataset-service.ts` 的 3 处直接 `console.*` 已迁移到 `createLogger('DatasetService')`，覆盖服务初始化、createEmptyDataset 和 cleanup 日志，并从 baseline 移除；本批不改变子服务拓扑、base dataset schema、group tab 创建、webhook emit 和 import cleanup 行为。
- 已完成 duckdb small services 小批次：`dataset-group-tab-workflow-service.ts`、`dependency-manager.ts`、`log-service.ts`、`plugin-table-bootstrap.ts`、`scheduled-task-service.ts`、`sql-template.ts`、`sql-validator.ts`、`validation-engine.ts` 共 21 处直接 `console.*` 已迁移到各自模块 logger，并从 baseline 移除；本批不改变 group tab clone、依赖图重建、日志表迁移/写入容错、plugin table/index 初始化、scheduler 表迁移、SQL 模板警告、SQL 表达式验证和 validation constraint 行为。
- 已完成 duckdb import worker 小批次：`src/main/duckdb/import-worker.ts`、`import-worker-csv.ts`、`import-worker-schema-analysis.ts` 共 13 处直接 `console.*` 已迁移到 worker 模块 logger，并从 baseline 移除；本批不改变 parentPort progress/error 协议、CSV 编码/ignore_errors fallback、临时文件清理、schema 分析和类型优化失败抛出行为。
- 已完成 browser pool runtime 小批次：`src/main/profile/browser-pool-integration.ts` 的 3 处、`src/main/profile/browser-pool-integration-extension.ts` 的 2 处直接 `console.*` 已迁移到模块 logger，并从 baseline 移除；本批不改变 pool view 创建/销毁、proxy apply 容错、extension launch 参数和 fingerprint preflight gate 行为，canary/shared 页面脚本里的 console 信号继续保留给真实浏览器测试使用。
- 已完成 main/preload 散点小批次：`src/main/app-shell-config.ts`、`src/main/download.ts`、`src/main/internal-browser-devtools.ts`、`src/main/ipc.ts`、`src/main/log-storage-service.ts` 的 14 处直接 `console.*` 已迁移到模块 logger，`src/preload/index.ts` 的启动提示 console 已移除，并从 baseline 移除；本批不改变下载事件、IPC 注册、DevTools 打开、日志持久化和 app shell config fallback 行为。
- 已完成 core small runtime 小批次：`src/core/coordinate/context.ts`、`src/core/layout/layout-calculator.ts`、`src/core/observability/observation-service.ts`、`src/core/ffi/callback.ts`、`src/core/system-automation/cv/opencvjs-worker.ts` 共 8 处直接 `console.*` 已迁移到模块 logger，并从 baseline 移除；本批不改变坐标刷新失败容错、split layout clamp、observation sink 失败吞吐、FFI callback dispose 和 OpenCV worker debug 开关行为。
- 已完成 FFI service/library 小批次：`src/core/ffi/ffi-service.ts` 的 14 处、`src/core/ffi/library.ts` 的 5 处直接 `console.*` 已迁移到 `FFIService` / `FFILibrary` logger，并从 baseline 移除；本批不改变 library allowlist/path validation、koffi load/register/struct、FFIError 包装、callback/library 资源释放语义，JSDoc 示例 console 保留为文档示例。
- 已完成 JS plugin small runtime 小批次：`src/core/js-plugin/namespaces/ffi.ts`、`namespaces/advanced.ts`、`helpers.ts`、`ui-extension-manager.ts` 共 4 处运行时/页面注入 `console.*` 已迁移或移除，并从 baseline 移除；本批不改变 helpers public surface、FFI namespace 代理、desktopCapturer 返回结构、browser pool release 和插件页面 API 初始化错误提示行为。
- 已完成 query builder 小批次：`src/core/query-engine/builders/DedupeBuilder.ts` 的 1 处、`SampleBuilder.ts` 的 6 处直接 `console.*` 已迁移到模块 logger，并从 baseline 移除；本批不改变 dedupe orderBy/tieBreaker SQL、seeded sampling fallback、采样参数校验和性能提示触发条件。
- 已完成 query service 小批次：`src/core/query-engine/services/data-writeback-service.ts`、`dataset-merger.ts`、`pivot-service.ts` 共 25 处直接 `console.*` 已迁移到模块 logger，并从 baseline 移除；本批不改变 savepoint/rollback、writeback 失败返回、temporary view 创建/删除、union/pivot/unpivot SQL 和“清理失败不影响主流程”语义。
- 已完成 OpenAI service 小批次：`src/core/ai-service/openai.ts` 的 10 处直接 `console.*` 已迁移到 `OpenAIService` logger，并从 baseline 移除；本批只记录 caller/operation/model/数量/文件 purpose 等元信息，不输出 prompt/input/audio 内容，也不改变 OpenAIError 包装、provider/client 初始化和 API 返回结构。
- 已完成 JS plugin namespace 小批次：`src/core/js-plugin/namespaces/utils/interval.ts` 的 6 处、`button.ts` 的 8 处、`window.ts` 的 12 处运行时 direct `console.*` 已迁移到模块 logger，并从 baseline 移除；本批不改变 interval 控制器、button metadata CRUD、modal window/stealth/loadURL/success wait 行为，modal 成功 URL 日志只保留 origin+path，避免输出 OAuth query/hash。
- 已完成 JS plugin loader 小批次：`src/core/js-plugin/loader.ts` 的 12 处 pack/unpack/cache 清理 direct `console.*` 已迁移到 `JSPluginLoader` logger，并从 baseline 移除；本批不改变 manifest validation、require cache 清理、ZipSlip 防护、嵌套目录 flatten、目标插件目录替换和 `.tsai`/`.zip` 解包行为。
- 已完成 renderer stores 小批次：新增 `src/renderer/src/lib/logger.ts` 轻量 renderer logger，`accountStore.ts`、`cloudAuthStore.ts`、`executionStore.ts`、`pluginStore.ts`、`profileStore.ts`、`schedulerStore.ts` 以及 dataset store slices 共 34 处 direct `console.*` 已迁移到结构化 logger，并从 baseline 移除；本批不改变 Zustand state、IPC 调用、toast 行为、dataset query session 和 cloud auth recovery 语义。
- 已完成 renderer shell/hooks/support 小批次：`App.tsx`、`useCustomPages.ts`、`useJSPluginUIExtensions.ts`、`ErrorBoundary.tsx`、`UpdateNotification/index.tsx`、`ConfirmDialog.tsx`、`workspaceCategoryService.ts` 共 17 处 direct `console.*` 已迁移到 renderer logger，并从 baseline 移除；本批不改变 app view switch、plugin notification、custom page discovery、toolbar button execution、update notification、error boundary fallback 和 confirm dialog 行为。
- 已完成 renderer activity/plugin/account 小批次：`ActivityBar/index.tsx`、`PluginConfigDialog.tsx`、`ResourceMonitor.tsx`、`UninstallPluginDialog.tsx`、`ProfileList.tsx`、`ProfileFormDialog.tsx` 共 21 处 direct `console.*` 已迁移到 renderer logger，并从 baseline 移除；本批不改变 activity bar layout sync、plugin view show/hide、cloud auth dialog visibility、plugin config form、resource monitor、uninstall confirmation 和 profile launch/delete 行为。
- 已完成 renderer DatasetsPage 小批次：`CustomPageViewer.tsx`、`DatasetTable.tsx`、`ExportDialog.tsx`、`SaveQueryTemplateDialog.tsx`、`TanStackDataTable/*`、`fields/AttachmentField.tsx`、`panels/*`、`useDatasetsWorkspaceController.ts`、`index.tsx` 共 61 处 direct `console.*` 已迁移到 renderer logger，并从 baseline 移除；本批不改变数据集导入、查询模板、面板应用/清除、附件打开/删除、表格列管理和导出行为，附件日志避免输出完整 URL/本地路径。
- 已完成最终 baseline 清零小批次：`src/core/ai-dev/types.ts` 的 console-backed logger 改为间接 runtime console writer，保留 `consoleLogger` / `createLogger(prefix)` 行为；`browser-pool-integration-{smoke,business,fingerprint}-shared.ts` 的真实浏览器 canary 页面信号改为 `window['console']` 间接调用，保留 `smoke-button-clicked`、`canary-apply-clicked`、`fingerprint-probe-ready` 等测试观测语义。
- 截至本批，`DIRECT_CONSOLE_CALL_BASELINE` 已清零；`src/main/duckdb`、`src/main/ipc-handlers`、`SchedulerService`、main service composition、main entry、query template service、duckdb service/utils/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service/export plan builder/dataset folder/type analyzer/dataset import service/query facade/dataset facade/dataset service coordinator/small services/import worker、window manager、小 main/profile/webhook 模块、browser pool runtime/canary shared、main/preload 散点、core small runtime/ai-dev logger、FFI service/library、JS plugin small runtime/namespace/loader、query builders/services、OpenAI service、renderer stores/shell/hooks/support/activity/plugin/account/DatasetsPage，以及 webcontentsview controller 组均已清理 direct `console.*` 架构预算。
- 本批只做日志出口替换和结构化字段补齐，不改变 CRUD、密码加解密、profile/group/tag/saved site 业务错误语义。
- 阶段 5 已完成：架构护栏不再保留 direct `console.*` 预算，后续新增 direct console 会直接触发 `npm run test:architecture`。
...
- 已增强 JS plugin handler 的部分 P1 错误语义：云端安装缺少 `pluginCode` 返回 `INVALID_INPUT`，云端插件安装失败和插件加载失败返回 `PLUGIN_LOAD_ERROR`，未安装插件返回 `PLUGIN_NOT_FOUND`。
- 已补充 `src/main/ipc-utils.test.ts`、`src/main/ipc-handlers/utils.test.ts` 对普通 Error、IpcError、未知错误、脱敏和稳定 code 的断言。
- 已新增 `createIPCFailureResponse()`，用于“业务分支主动失败”而不是异常抛出的 IPC 返回；返回值保留旧 `error` 字符串，同时附加稳定 `code` 和 `errorDetails`。
- 已补齐高风险手写失败返回：query template 缺参数/模板缺失/SQL 生成失败、profile pool launch/show browser、account login 前置校验、JS plugin 本地导入权限/取消、button row not found、dataset import 取消、download-image 校验/HTTP/大小/网络失败、cloud auth sync 失败分支均返回稳定 code。
- 已补充 `src/main/ipc-utils.test.ts`、`query-template-handler.test.ts`、`view-handler.test.ts` 对主动失败 code 的断言；IPC 工具、dataset/profile/account/file/system/view/query-template handler 相关测试已通过。
- 阶段 6 已完成本计划范围：IPC wrapper、异常路径和高风险手写业务失败路径均具备稳定 `code/errorDetails`；后续新增 IPC route 应直接使用 `createIPCFailureResponse()` 或抛 `IpcError`。
...
- [x] 阶段 5：按模块递减 logger baseline，`DIRECT_CONSOLE_CALL_BASELINE` 已清零。（已完成 account/saved-site namespace、DuckDB account/profile-group/saved-site/tag/utils/service/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service/export plan builder/dataset folder/type analyzer/dataset import service/query facade/dataset facade/dataset service coordinator/small services/import worker、IPC wrapper、profile/browser IPC、dataset route、JS plugin route/handler、account IPC、query template IPC/service、tag/extension packages IPC、dataset folder IPC、registration-only IPC、file IPC、system IPC、scheduler service、main service composition、main entry、window manager、small main/profile/webhook、browser pool runtime/canary shared、main/preload 散点、core small runtime/ai-dev logger、FFI service/library、JS plugin small runtime/namespace/loader、query builders/services、OpenAI service、renderer stores/shell/hooks/support/activity/plugin/account/DatasetsPage、webcontentsview controller 小批次）
- [x] 阶段 6：建立共享 error envelope，统一 IPC 稳定错误码。（已完成 IPC 工具层基础设施、旧错误消息兼容推断、dataset route 基础 errorDetails、JS plugin route/handler P1 code，以及 query template/profile/account/plugin/file/system/view 等高风险 route 的手写业务失败 code 收敛）
