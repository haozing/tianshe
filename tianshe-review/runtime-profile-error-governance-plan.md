...
- [ ] 阶段 5：按模块递减 logger baseline。（已完成 account/saved-site namespace、DuckDB account/profile-group/saved-site/tag、IPC wrapper、profile/browser IPC、dataset route、JS plugin route/handler、account IPC、query template IPC、tag/extension packages IPC、dataset folder IPC 小批次）
