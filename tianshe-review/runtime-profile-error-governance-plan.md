...
- 截至本批，`src/main/duckdb` 生产代码已基本完成 direct console baseline 清理；`src/main/ipc-handlers` 生产代码已无直接 `console.*`，`SchedulerService`、main service composition、main entry、query template service、duckdb service/utils/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service/export plan builder/dataset folder/type analyzer/dataset import service/query facade/dataset facade/dataset service coordinator/small services/import worker、window manager、小 main/profile/webhook 模块、browser pool runtime、main/preload 散点、core small runtime、FFI service/library、JS plugin small runtime/namespace/loader、query builders/services、OpenAI service、renderer stores，以及 webcontentsview controller 组也已清零；后续重点转向 profile/browser-pool canary/shared 页面脚本信号、ai-dev console logger 和 renderer 组件/ hooks 少量 direct console。
- 本批只做日志出口替换和结构化字段补齐，不改变 CRUD、密码加解密、profile/group/tag/saved site 业务错误语义。
- 阶段 5 仍保留为未完成：仓内还有 profile/browser-pool canary/shared 页面脚本信号、ai-dev console logger 实现、renderer 组件/hooks 少量 direct console 等剩余热点，后续继续按模块递减，不做全仓一键替换。
