...
- 截至本批，`src/main/ipc-handlers` 生产代码已无直接 `console.*`，`SchedulerService`、main service composition、main entry、query template service、duckdb service/utils/dataset storage/dataset metadata/dataset record mutation/dataset query/dataset schema/dataset export writer/export service、window manager、小 main/profile/webhook 模块，以及 webcontentsview controller 组也已清零；后续重点转向其他 duckdb dataset 子服务/facade/import-worker、profile/browser-pool integration 和 renderer 少量 direct console。
